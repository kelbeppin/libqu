...
static void gl1__apply_surface(struct qu__texture_data const *texture)
{
    if (texture) {
        _GL_CHECK(priv.glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, texture->priv[1]));
        _GL_CHECK(glViewport(0, 0, texture->image.width, texture->image.height));
        priv.bound_surface = texture->priv[1];
    } else {
        _GL_CHECK(priv.glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0));
        _GL_CHECK(glViewport(0, 0, priv.w_display, priv.h_display));
        priv.bound_surface = 0;
    }
}
...
static void gl1__set_texture_smooth(struct qu__texture_data *texture, bool smooth)
{
    GLuint id = (GLuint) texture->priv[0];
...
static void gl1__create_surface(struct qu__texture_data *texture)
{
    GLuint fbo;
    GLuint depth;
    GLuint color;
    ...
    _GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, texture->image.width, texture->image.height,
    ...
    texture->priv[0] = color;
    texture->priv[1] = fbo;
    texture->priv[2] = depth;

    _GL_CHECK(priv.glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, priv.bound_surface));
    _GL_CHECK(glBindTexture(GL_TEXTURE_2D, priv.bound_texture));
}

static void gl1__destroy_surface(struct qu__texture_data *texture)
{
    GLuint color = texture->priv[0];
    GLuint fbo = texture->priv[1];
    GLuint depth = texture->priv[2];
